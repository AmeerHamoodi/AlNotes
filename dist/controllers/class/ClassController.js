module.exports = class ClassController {
    ...
    _newClass() {
        ...
        ipcMain.on("newClass", (eve, args) => {
            if (storage.createClass(args) === false)
                return this.mainWindow.webContents.send(
                    "classThread:error",
                    `Class ${args} already exists`
                );
            storage.saveAll();
    ...
    _newClassItem() {
        ...
        ipcMain.on("createNewClassItem:textbook", (ev, args) => {
            ...
            mainWindow.webContents.send(
                "classContent",
                storage.getClassByName(args.className)
            );
        });
    ...
    _deleteClass() {
        ...
        ipcMain.on("deleteClass", (ev, args) => {
            if (storage.deleteClass(args) === false)
                return this.mainWindow.webContents.send(
                    "classThread:error",
                    `Class ${args} doesn't exists`
                );
    ...
    _unitListeners() {
        ...
        ipcMain.on("deleteUnit", (eve, args) => {
            ...
            if (storage.deleteUnit(args.className, args.unitName) === false)
                return this.mainWindow.webContents.send(
                    "classThread:error",
