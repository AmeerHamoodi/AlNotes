...
class NotesStore extends DefaultStore implements NotesStoreInterface {
    ...
    _listenNotes() {
        ipcRenderer.on(
            "getNotes:response",
            (event: object, args: NoteFrontInterface[]) => {
                try {
                    if (!Array.isArray(args))
                        throw new ResponseError(
                            "Invalid response for 'getNotes:response'"
                        );

                    runInAction(() => {
                        this.notes = args.map((item) => {
                            const note: NoteFrontInterface = new NoteFront(
                                item.name,
                                item.id
                            );
                            note.deleteFunction = () => {
                                if (
                                    confirm(
                                        "Are you really sure you want to delete this note? This is irreversible!"
                                    )
                                )
                                    this.deleteNote(
                                        this.currentClass,
    ...
    public getNotes(className: string, unitName: string) {
        try {
            if (typeof className !== "string" || typeof unitName !== "string")
                throw new StoreError(
                    "Invalid className, unitName, or noteName!"
                );
    ...
    public createNote(className: string, unitName: string, noteName: string) {
        try {
            if (
                ...
                throw new StoreError("Invalid className or unitName!");
    ...
    public deleteNote(className: string, unitName: string, id: string) {
        try {
            if (
                ...
                throw new StoreError("Invalid className, unitName, or id!");
