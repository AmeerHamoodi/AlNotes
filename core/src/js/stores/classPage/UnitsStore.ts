        ...
        ipcRenderer.on(
            ...
            (event: object, data: object[]) => {
                try {
                    ...
                    const frontViewArray: UnitFrontInterface[] =
                        Array.isArray(data) &&
                        data.map(
                            (item: UnitFrontParams) =>
                                new UnitFront(item, this.currentClass)
                        );

                    runInAction(() => {
                        this.units = frontViewArray.map(
                            (unit: UnitFrontInterface) => {
                                unit.deleteFunction = () => {
                                    if (
                                        confirm(
                                            "Are you sure you want to delete this unit? This is irreversiable"
                                        )
                                    )
                                        this.deleteUnit(
                                            this.currentClass,
    ...
    public createUnit(className: string, unitName: string) {
        try {
            if (
                ...
                throw new StoreError("Invalid unit name!");
    ...
    public deleteUnit(className: string, unitName: string) {
        try {
            if (typeof className !== "string" || typeof unitName !== "string")
                throw new StoreError("Invalid unit or class name");
