...
class ClassItemsStore extends DefaultStore implements ClassItemsInterface {
    ...
    _itemsListener() {
        ipcRenderer.on(
            "classContent",
            (event: object, args: ClassContentResponse) => {
                try {
                    if (typeof args !== "object" && args !== null)
                        throw new ResponseError("Invalid response");

                    const textbooks: ClassItemFrontInterface[] = args.textbooks.map(
                        (item) => {
                            const textbook: ClassItemFrontInterface = new ClassItemFront(
                                item.name,
                                item.link
                            );
                            textbook.deleteFunction = () => {
                                if (
                                    confirm(
                                        "Are you sure you want to delete this item? This is irreversiable"
                                    )
                                ) {
                    ...
                    const labs: ClassItemFrontInterface[] = args.labs.map(
                        (item) => {
                            const lab: ClassItemFrontInterface = new ClassItemFront(
                                item.name,
                                item.link
                            );
                            lab.deleteFunction = () => {
                    ...
                    const meetings: ClassItemFrontInterface[] = args.meetings.map(
                        (item) => {
                            const meeting: ClassItemFrontInterface = new ClassItemFront(
                                item.name,
                                item.link
                            );
    ...
    public createClassItem(
        ...
        try {
            if (
                !["textbook", "lab", "meeting"].includes(type) ||
                typeof className !== "string"
            )
                throw new StoreError("Invalid class name or type");

            if (!("name" in data) || !("link" in data))
                throw new StoreError("Name or link of item not included");

            if (data.name.length < 0 || !data.name.trim())
                throw new StoreError("Name is invalid");
            if (data.link.length < 0 || !data.link.trim())
                throw new StoreError("Link is invalid");
